...
do
  ...
  shp2pgsql -I -s EPSG:32626 "$shppath" "tp_${shpfile%.*}" | psql -U $PSQLUSER $PSQLDB
done
...
do
  shpfile=`basename "$shppath"`
  PYTHONPATH=./layer_importer:$PYTHONPATH ./layer_importer/bin/layer_importer taesp_ahrc_2007 postgis_viewer_taesp "../arcims_data/taesp_translated/$shpfile" EPSG:32626
done
